...
AMA.flags = AMA.flags || {};
AMA.flags.debugCountyMarker = AMA.flags.debugCountyMarker || false;
...
async function ama_bootstrap(){
  ...
  function pointStyle(kind){
    return { radius:4, weight:1, opacity:1, fillOpacity:0.6 };
  }

  function addPointGroup(key, gj){
    if(!gj) return;
    const layer = L.geoJSON(gj, {
      pointToLayer: (f, latlng)=> L.circleMarker(latlng, pointStyle(key))
    });
  ...
  addPointGroup('wind', windFC);
  addPointGroup('solar', solarFC);
  addPointGroup('dams', damsFC);
