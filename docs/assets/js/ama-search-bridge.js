...
(function AMA_SEARCH_BRIDGE(){
  ...
  const expectedCounties=['تایباد','خواف','زاوه','مشهد','نیشابور'].map(norm);
  const expectedWind=['البلاغ','باراکوه','تق‌قز سفلی','حصاریزدان','خرگرد','خواف','رهنه','سراب','سنگال‌آباد','سنگان','طرح توسعه چخماق','طرح توسعه کنگ اولیا','عباسی‌آباد','عبدل‌آباد','فهندر','فیندر','قادری‌آباد','مهرآباد','نشتیفان','نصر‌آباد','نوده','کودکان','کوه‌آباد'].map(norm);

  function getCountyFeatures(){
    if (window.__AMA_MAP?.countiesGeo?.features?.length) return window.__AMA_MAP.countiesGeo.features.slice();
  ...
  window.__amaSearch={ stats:()=>stats(), rebuild:()=>{ buildIndex(); log('[AMA-search] reindexed:', stats()); }, focusCountyByName, focusWindSiteByName, clearHL };
})();
